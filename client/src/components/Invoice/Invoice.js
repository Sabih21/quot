...
const Invoice = () => {
    ...
    const [customerList, setCustomerList] = useState([]) // Existing clients list
    const [currentInvoice, setCurrentInvoice] = useState({ client: null })
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        if (pendingCustomer && !customerList.some(c => c.name === pendingCustomer.name)) {
            try {
                const response = await axios.post("http://localhost:5000/clients", pendingCustomer);
                const savedCustomer = response.data;

                // Update customer list with the newly added customer
                setCustomerList([...customerList, savedCustomer]);

                // Assign the new customer to the invoice
                setCurrentInvoice({ ...currentInvoice, client: savedCustomer });
            } catch (error) {
                            ...
                            <Container>
                                ...
                                <div style={client ? { display: 'none' } : { display: 'block' }}>
                                    ...
                                    <ClientDropdown
                                        ...
                                        setClient={(client) => setCurrentInvoice({ ...currentInvoice, client })}
                                        onChangeClient={(event, value) => setClient(value)}
                                        onClientChange={(value) => setCurrentInvoice({ ...currentInvoice, client: value })}
                                        setNewClient={setPendingCustomer}
                                    />
